...
def task_udf(pdf_iter: Iterable[pd.DataFrame],
             ...
             driver_ip: str,
             filepath: str) -> Iterable[pd.DataFrame]:
    """
    ...
    # Extract number of trials from the dummy DataFrame.
    pdf = next(pdf_iter)
    num_trials = pdf["num_trials"].iloc[0]
...
if __name__ == "__main__":
    ...
    dummy_rdd = spark.sparkContext.parallelize([(x,) for x in trials_per_partition], numSlices=num_partitions)
    df = dummy_rdd.toDF(schema=["num_trials"])
    ...
    result_df = df.mapInPandas(lambda pdf_iter: 
                               task_udf(pdf_iter, 
