...
object Util {
  def createSparkSession(): SparkSession = {
    ...
    val builder = SparkSession.builder()
        ...
        .config("spark.rapids.memory.pinnedPool.size", "16g")
        .config("spark.rapids.memory.gpu.allocFraction", "0.9")
        .config("spark.rapids.sql.castStringToInteger.enabled", "true")
        .config("spark.executor.extraJavaOptions", "-ea -Duser.timezone=UTC -Dai.rapids.cudf.prefer-pinned=true")
        .config("spark.task.resource.gpu.amount", "0.08")
        .config("spark.executor.resource.gpu.amount", "1")
        .config("spark.rapids.shuffle.ucx.enabled", "false")
