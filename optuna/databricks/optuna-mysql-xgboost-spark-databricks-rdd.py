...
def task(num_trials: int = 100):
    ...
    This example uses Spark RDDs to distribute the trials across the cluster.
    """
...
partitions = partition_trials(total_trials, total_tasks)
# Map task onto RDD partitions and collect results.
rdd = spark.sparkContext.parallelize(partitions, numSlices=total_tasks)
results = rdd.map(task).collect()
