...
create an `init.sh` scripts with contents:   
   ```bash
   #!/bin/bash
   sudo wget -O /databricks/jars/rapids-4-spark_2.12-24.08.0.jar https://repo1.maven.org/maven2/com/nvidia/rapids-4-spark_2.12/24.08.0/rapids-4-spark_2.12-24.08.0.jar
   ```
...
6. Now select the “Spark” tab, and paste the following config options into the Spark Config section.
   ...
   for Apache Spark [descriptions](https://nvidia.github.io/spark-rapids/docs/configs.html) for each config.
    ...
    spark.executorEnv.PYTHONPATH /databricks/jars/rapids-4-spark_2.12-24.08.0.jar:/databricks/spark/python
    ```
